  ...
  const columns: ColumnDef<Teacher>[] = [
    {
      ...
      cell: ({ row }) => (
        <Link href={`/teachers/${row.original.id}`} passHref>
    ...
    {
      accessorFn: (teacher) => {
        const grades = teacher.classes
          ? teacher.classes
              .map((cls) => cls.grade?.name || "")
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        const s = new Set(courses); // Get unique course names
        const uniqueCourses = Array.from(s);
      ...
      //   filterFn: (row, columnId, filterValue) => {
      //     const courses = row.getValue(columnId) as Array<string>;
      //     const test = courses.some(
      //       (course) => course.toLowerCase() === filterValue.toLowerCase()
      //     );
      //     console.log("gradeshjjj", courses);
      //     console.log("dhd", test);
      //     console.log("filterValue", filterValue);
      //     if (filterValue === "none") {
      //       return courses.some((course) => course === undefined);
      //     }
      //     return courses.some(
      //       (course) => course.toLowerCase() === filterValue.toLowerCase()
      //     );
      ...
      filterFn: (row, columnId, filterValue) => {
        ...
        console.log(`--- Evaluating row ${row.id} ---`);
        console.log("Courses:", courses);
        console.log("Filter value:", filterValue);

        if (filterValue === "none") {
          const result = courses.length === 0;
          console.log("'none' filter result:", result);
          return result;
        }

        const result = courses.some(
          (course) => course.toLowerCase() === filterValue.toLowerCase()
        );
        console.log("Regular filter result:", result);
        return result;
      },
    },
        ...
        [
          ...new Set(
            row.original.classes
              ?.map((classs) => classs.course?.name)
              .filter(Boolean) ?? []
          ),
        ].join(", "),
        row.original.classes?.map((classs) => classs.grade?.name).join(",") ??
          "",
        row.original.classes?.map((classs) => classs.name).join(",") ?? "",
      ]);
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
          <div>
            ...
            <Select
              ...
              onValueChange={(event) => {
                if (event === "all") {
                  ...
                  if (event === "none") {
                    table.getColumn("course")?.setFilterValue("none");
                    setSelectedCourse(undefined);
                  } else {
            ...
            >
              ...
              <SelectContent>
                <SelectItem value={"none"}>{t("No course")}</SelectItem>
                {courses.map((course) => (
